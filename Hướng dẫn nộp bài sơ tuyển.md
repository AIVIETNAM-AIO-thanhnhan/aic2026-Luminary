...
> Tài liệu chính thức của Ban tổ chức. Nội dung được giữ nguyên, chỉ định dạng lại
> cho dễ đọc.

## Mục lục

- [1. Các loại truy vấn](#1-các-loại-truy-vấn)
- [2. Các gói truy vấn](#2-các-gói-truy-vấn)
- [3. Yêu cầu kết quả](#3-yêu-cầu-kết-quả)
- [4. Quy chuẩn định dạng CSV](#4-quy-chuẩn-định-dạng-csv)
- [5. Hướng dẫn tạo file CSV](#5-hướng-dẫn-tạo-file-csv)
- [6. Nộp kết quả cho gói truy vấn](#6-nộp-kết-quả-cho-gói-truy-vấn)
- [7. Đánh giá và xếp hạng](#7-đánh-giá-và-xếp-hạng)
- [8. Bảng tóm tắt](#8-bảng-tóm-tắt)
- [9. Năm lỗi thường gặp nhất](#9-năm-lỗi-thường-gặp-nhất)
- [10. Checklist trước khi nộp](#10-checklist-trước-khi-nộp)
- [11. Khi gặp vấn đề](#11-khi-gặp-vấn-đề)
...
Trong vòng sơ tuyển, BTC sẽ cung cấp lần lượt các gói câu truy vấn theo nhiều đợt.
Với mỗi gói câu truy vấn, đội thi cần trả về kết quả tương ứng cho BTC và nộp trên
hệ thống **Codabench**.

Với mỗi gói câu truy vấn, BTC sẽ cung cấp một danh sách các câu truy vấn trong từng
file text. Ví dụ trong đợt 1, BTC cung cấp gói gồm 4 câu truy vấn `query-1-kis`,
`query-2-kis`, `query-3-qa`, `query-4-trake` tương ứng với nội dung trong 4 file:
...
| Hậu tố | Dạng truy vấn |
|---|---|
| `kis` | Câu truy vấn dạng Textual KIS |
| `qa` | Câu truy vấn dạng Q&A |
| `trake` | Câu truy vấn dạng TRAKE |

---

## 3. Yêu cầu kết quả

Đối với mỗi câu truy vấn, đội thi cần nộp tương ứng một file `.csv`
(*comma-separated values file*) với mỗi dòng tương ứng với một lần đội dự đoán kết
quả. **Đội thi có thể nộp file tối đa 100 dòng.**

Kết quả trên mỗi dòng của đội có format theo từng loại truy vấn:

### 3.1. Textual Known Item Search (Textual KIS)
...
### 3.3. Temporal Retrieval and Alignment of Key Events (TRAKE)

**Format:** `<Tên file video>, <Frame ID_1>, <Frame ID_2>, ..., <Frame ID_N>`

Trong đó:

- `Frame ID_1`, `Frame ID_2`, …, `Frame ID_N` là các keyframe tương ứng với **N
  events** trong chuỗi sự kiện
...
## 4. Quy chuẩn định dạng CSV

> [!WARNING]
> **Lưu ý QUAN TRỌNG cho học sinh THPT — CSV ≠ Excel**
>
> Đây là hai định dạng file **hoàn toàn khác nhau**:
>
> - **File CSV (`.csv`)**: là file văn bản thuần túy, chỉ chứa dữ liệu được phân
>   cách bằng dấu phẩy
...
| Từ công cụ | Thao tác |
|---|---|
| Excel | `File` → `Save As` → chọn **CSV (Comma delimited) (*.csv)** |
| Google Sheets | `File` → `Download` → **Comma Separated Values (.csv)** |
| Notepad | Gõ trực tiếp theo format và lưu với đuôi `.csv` |
| Text editor khác | VS Code, Sublime Text, Notepad++ |
...
| Delimiter | Dấu phẩy `,` |
| Line ending | CRLF (`\r\n`) hoặc LF (`\n`) |
| Header row | **Không có** — file CSV bắt đầu trực tiếp bằng dữ liệu |

### Xử lý ký tự đặc biệt

> [!IMPORTANT]
> Dấu ngoặc kép chỉ **BẮT BUỘC** khi answer chứa các ký tự đặc biệt. Nếu answer
> đơn giản không có ký tự đặc biệt, có thể bỏ qua dấu ngoặc kép.
...
**An toàn nhất:** để tránh nhầm lẫn, có thể **luôn** đặt dấu ngoặc kép cho tất cả
answer trong Q&A. Cả hai cách đều được hệ thống chấp nhận.

---

## 5. Hướng dẫn tạo file CSV

*(dành cho học sinh THPT)*

### Phương pháp 1 — Microsoft Excel
...
## 6. Nộp kết quả cho gói truy vấn

Mỗi đội thi cần đăng ký một tài khoản trên **Codabench** và đăng ký tham gia vào
cuộc thi. BTC sẽ duyệt cho đội tham gia vào cuộc thi nếu tên hoặc thông tin tài
khoản Codabench trùng với thông tin đội thi đã đăng ký trước đó với BTC.

### Cách chuẩn bị file nộp

| Bước | Thao tác |
|---|---|
| 1 | Tạo thư mục có tên `submission` |
| 2 | Đặt tất cả file CSV kết quả vào trong thư mục `submission` |
| 3 | Nén thư mục `submission` thành file `.zip` |
| 4 *(tùy chọn)* | Đổi tên file zip thành tên phù hợp (ví dụ: `team_ABC_round1.zip`) |
...
- Đội thi có thể xem cách đánh giá tại tab **Evaluation** để biết thêm thông tin về
  cách tính điểm
...
Kết quả đánh giá trên **Public Leaderboard** chỉ tính dựa trên **50%** đáp án của
BTC. Kết quả cuối cùng của đội nộp sẽ được tính trên **100%** đáp án và dùng để xếp
hạng vòng sơ tuyển tại **Private Leaderboard**.

### Phương pháp tính điểm

Mỗi gói truy vấn, các đội được phép nộp kết quả **tối đa 3 lần**. Kết quả được dùng
để xếp hạng là kết quả đội nộp **lần cuối cùng**.
...
| ✍️ Text editor an toàn | Notepad, VS Code, Google Sheets | Excel cần `Save As` → CSV |

---

## 9. Năm lỗi thường gặp nhất
